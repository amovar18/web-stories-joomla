...
class WebstoriesController extends ApiController
{
  ...
  public function get_all_stories()
  {
    ...
    $query = $db->getQuery(true);
    $query
      ->select($db->quoteName(array('a.id', 'a.markup', 'a.post_date', 'a.title', 'a.modified_date', 'a.created_by', 'a.published', 'a.post_content_filtered', 'a.story_description', 'a.featured_media_url', 'b.username')))
      ->from($db->quoteName('#__webstories', 'a'))
      ->join('INNER', $db->quoteName('#__users', 'b') . ' ON ' . $db->quoteName('a.created_by') . ' = ' . $db->quoteName('b.id'))
    ...
    $totalStoriesByStatus = ['all' => $published + $draft, 'publish' => $published, 'draft' => $draft];
  ...
  public function getAllAuthors()
  {
    ...
    $users = [];
    foreach ($items as $item) {
      array_push($users, array("id" => $item['id'], "name" => $item['username']));
    }
